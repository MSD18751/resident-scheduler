...
def create_model(data, policy_type="4+1", model_np=52, model_v=1):
    ...
    # define a dict to store possible year levels
    ...
    # Define parameters
    # model.naught_p is the num of weeks before a resident has to return
    naught_p = {}
    for p in P:
    ...
    for g in model.data["Sets"].index.values:
        for c in list(data["Units"].query(
                "Unit_type == 'Clinic'").index.values):
    ...
    for y in list(model.data["Residents"]["Year_Level"].unique()):
        for u in model.data["Units"].index:
            for i in list(("Min", "Max")):
                model_Phi_y_u[(y, u, i)] = model.data["Units"].loc[u, str(
                    "R"+str(y)+i)]
    ...
    # ---Define variables---
    # resident r does/doesn't work unit u week t
    model.X = pyomo.Var(
        model.R, model.U, model.T, domain=pyomo.Binary, initialize=0)
    # resident r does/doesn't START rotation unit u week t
    model.W = pyomo.Var(
        model.R, model.U, model.T, domain=pyomo.Binary, initialize=0)
    # unit u does/doesn't START a rotation in week t
    model.delta = pyomo.Var(
        model.U, model.T, domain=pyomo.Binary)

    # ---Define objective function---
    def obj(model):  # objective to satisfy resident vacation preferences
        return sum(model.X[r, u, t] * model.psi_r_t[r, t]
                   for r in model.R for u in model.U for t in model.T)

    # a maximization problem of the function defined above
    ...
    def Cons9(model, r, c, t):
        for r in model.R:
            for c in model.C:
                for t in list(range(
                        1, model.naught_p[policy_type] + model.s + 1)):
                    sumofX = 0
                    q = 0
                    while t + (q * (model.naught_p[policy_type] + (
                            model.s))) <= model_np:
                                sumofX = sumofX + model.X[r, c, t + (
                                    q * (model.naught_p[policy_type] + (
                                            model.s)))]
                                q = q + 1
                    return sumofX >= model_alpha_u[(
                        c, "min")] * model.W[r, c, t]

    model.ClinicRotation = pyomo.Constraint(model.R, model.C, list(
        range(1, naught_p[policy_type] + s + 1)), rule=Cons9)
    ...
    model.ClinicHistory = pyomo.Constraint(
        model.R, model.T, model.C, model.G, rule=Cons11)

    def Cons12(model, t, c):
        return sum(sum(model.I_r_g[r, g] * (
                            model.h_g_c[g, c] * model.W[r, c, t])
                       for g in model.G) for r in model.R) >= model.m

    model.ClinicVerification = pyomo.Constraint(list(
        range(1, naught_p[policy_type] + 1)), model.C, rule=Cons12)

    # make sure at least 2 residents are working unit u every week
    def Cons14(model, t, u):
        return model_Phi_y_u[(y, u, "Min")] <= sum(
            model.X[r, u, t] for r in model.R) <= model_Phi_y_u[(y, u, "Max")]
    # the assignment constraint for number of residents working
    model.Residents = pyomo.Constraint(model.T, model.U, rule=Cons14)
    # makes sure that every resident is assign to max 1 place each week

    def Cons15(model, r, t):
        return sum(model.X[r, u, t] for u in model.U) <= 1
    ...
    # model.NoYoungFolks = pyomo.Constraint(
    # model.Ri, model.U, model.T, rule = Cons16)
